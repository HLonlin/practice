...
const wechat = {
    ...
    initJssdk: function(callback) {
        ...
        //服务端进行签名
        if ((this.userAgent.toLowerCase().match(/wxwork/i) == 'wxwork')) {
            // 企业微信签名
            axios.post(api.getWorkWechatGlobalSign, { url: url }).then(res => {
                if (res.data) {
                    wx.config({
                        ...
                        jsApiList: ["getLocation", "openLocation", "agentConfig"] // 必填，需要使用的JS接口列表
                    });
                    wx.ready(function() {
                        axios.post(api.getWorkWechatInvokeSign, {
                            url: url,
